...
interface StoreContextType {
  ...
  setSelectedStore: (l: Location | null) => void;
...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Selected Store State (Where actions happen)
  ...
  const [kassaBrands, setKassaBrands] = useState<string[]>(['Epson', 'Sunmi', 'Omnitech','NBA']);
  ...
  const fetchData = async () => {
    try {
      ...
      if(tData) setTransactions(tData.map((t: any) => ({
          ...
          storeId: t.store_id, // Load store_id
          user: t.user_name
      })));
  ...
  const login = async (u: string, p: string) => {
    try {
        ...
        if (data) {
            ...
            // Fetch latest locations to ensure ID validity
            const { data: locs } = await supabase.from('locations').select('*');
            if (locs) {
                const stores = locs.filter((l: any) => l.type === 'STORE' || l.type === 'WAREHOUSE');
                setLocations(stores); // Update state

                if (mappedUser.role === UserRole.ADMIN) {
                    // Admin defaults to first available store/warehouse
                    if (stores.length > 0) setSelectedStore(stores[0]);
                } else {
                    // Staff defaults to first ALLOWED store
                    const allowed = stores.find((s: any) => 
                        mappedUser.allowedStoreIds?.includes(s.id) || 
                        mappedUser.allowedWarehouseIds?.includes(s.id)
                    );
                    if (allowed) setSelectedStore(allowed);
                }
            }
            return true;
        }
  ...
  const addInvoice = (invoice: Invoice) => {
      const insertAsync = async () => {
          // ENSURE INVOICE IS LINKED TO SELECTED STORE
          const targetStoreId = selectedStore?.id || invoice.locationId || 'loc-1'; // Fallback to loc-1 only if strict mode fails
          ...
          const { error: invErr } = await supabase.from('invoices').insert([{
              ...
              location_id: targetStoreId, // Use resolved store ID
              status: invoice.status, parent_invoice_id: invoice.parentInvoiceId,
          ...
          for(const item of invoice.items) {
              ...
              if(prod) {
                  ...
                  // Update Global Stock (Optional, depending on business logic, but usually sum of local)
                  // For now, updating global stock as cache
                  const newTotal = prod.stock + change;
                  await supabase.from('products').update({stock: newTotal}).eq('id', prod.id);

                  // Update LOCAL Store Stock
                  const locId = targetStoreId;
          ...
          if (invoice.paymentMethod !== PaymentMethod.CREDIT) {
              ...
              await addTransaction({
                  ...
                  storeId: targetStoreId, // Ensure transaction is linked to store
  ...
  const addTransaction = async (t: Transaction) => {
      // Ensure storeId is present if not provided
      const finalStoreId = t.storeId || selectedStore?.id; 

      await supabase.from('transaction_logs').insert([{
          id: t.id, date: t.date, type: t.type, category: t.category, expense_category_id: t.expenseCategoryId,
          related_invoice_id: t.relatedInvoiceId, partner_id: t.partnerId, amount: t.amount, // Fix here
          description: t.description, source: t.source, bank_id: t.bankId, user_name: t.user,
          store_id: finalStoreId // Save location context
      }]);
      fetchData();
  };
  ...
  const addEmployee = async (e: Employee) => { await supabase.from('employees').insert([{
      ...
      store_id: e.storeId // Add store_id
  }]); fetchData(); };
  const updateEmployee = async (e: Employee) => { await supabase.from('employees').update({
      first_name: e.firstName, last_name: e.lastName, salary: e.salary, job_title: e.jobTitle, store_id: e.storeId
  ...
  return (
    <StoreContext.Provider value={{
      ...
      selectedStore, setSelectedStore, // EXPOSED
